...
// ── Run evaluation ─────────────────────────────────────────────────────────
async function runEval(): Promise<{ rowsInserted: number; errors: string[] }> {
  const errors: string[] = [];
  let rowsInserted = 0;
  ...
  try {
    ...
    for (const agent of agents) {
      try {
        ...
        rowsInserted++;
        console.log(
          `[eval-runner] ${agent.id} (${agent.displayName}): score=${scoreBps} bps, corpus=${CORPUS_VERSION}`
        );
      } catch (err) {
...
export async function triggerOneOff() {
  ...
  // Wait for completion
  const result = await job.waitUntilFinished(queue.events, 120_000);
...
const isMain = process.argv[1]?.endsWith("eval-runner.ts") || process.argv[1]?.endsWith("eval-runner.js");
if (isMain) {
  main().catch((err) => {
