...
- [x] Partner analytics endpoint returns valid JSON for a seeded partner. Verified: `GET /api/admin/partners/:code/analytics` returns 200 with correct shape (`code`, `windows.30d/60d/90d` with `orders`+`revShareUsd`, `topAgents` with `agentId`+`orders`+`revUsd`, `generatedAt`); 401 on bad/missing admin token; 404 on unknown code; e2e test in `e2e/partner-analytics.spec.ts` covers all paths; `npx tsc --noEmit` passes; `npm run build` succeeds (PRI-2291).
